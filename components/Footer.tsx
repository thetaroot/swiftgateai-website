...
function Footer() {
  ...
  return (
    <footer
      ...
      style={{ background: 'var(--color-balsamico)', paddingBottom: isMobile ? 'max(24px, env(safe-area-inset-bottom))' : undefined }}
    >
      <div className="max-w-7xl mx-auto">
        <div className={`flex flex-col items-center ${isMobile ? 'gap-5' : 'gap-6'}`}>
          ...
          <div className={`flex flex-col items-center text-center ${isMobile ? 'gap-1.5 px-2' : 'gap-2'}`}>
            ...
            >
              © {CURRENT_YEAR} Swiftgateai – {t.footer.rights}
            </div>
            <div
              className={`${isMobile ? 'text-[11px] leading-relaxed' : 'text-xs'} max-w-2xl`}
              style={{ color: 'rgba(234, 206, 170, 0.9)' }}
            >
              {t.footer.disclaimer}
            </div>
          </div>
